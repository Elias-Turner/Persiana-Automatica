...
DISTANCIA_LIMITE_SUP = 50.0  # NUEVO: Límite superior para detener subida (cm)
...
print(f"- Límite superior de seguridad: {DISTANCIA_LIMITE_SUP} cm")
...
try:
    ...
    while True:
        ...
        if estado_motor == MOTOR_SUBIENDO:
            # Detener si llega al tope superior
            if dist_sup is not None and dist_sup <= DISTANCIA_PARADA_SUP:
                print("[LÍMITE] Cortina alcanzó tope SUPERIOR")
                control_motor("detener")
            # NUEVO: Detener si la distancia es MAYOR al límite superior
            elif dist_sup is not None and dist_sup > DISTANCIA_LIMITE_SUP:
                print(f"[LÍMITE] ¡Distancia superior excedida ({dist_sup:.1f} cm > {DISTANCIA_LIMITE_SUP} cm)! Deteniendo motor por seguridad.")
                control_motor("detener")
